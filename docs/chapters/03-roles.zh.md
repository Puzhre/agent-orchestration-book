...
## Your Identity & Memory     ← 角色定义、性格
## Your Core Mission          ← 核心职责
## Key Rules You Must Follow  ← 行为约束
## Your Technical Deliverables ← 代码/报告模板
## Your Communication Style   ← 交互风格
```

**Tmux-Orchestrator的CLAUDE.md**：
一个716行的行为知识库，包含完整的角色层级、Git纪律、通信协议、反模式列表。

**优点**：灵活、可读性强、LLM天然理解
**缺点**：无强制执行力——Agent可以"偷懒"不遵守规则

### 方式二：代码强制边界（Tmux-Orchestrator / Overstory）
...
```
Architect(Hermes): 只管"做什么"
Executor(Codex):   只管"怎么做"
```
...
### 防线二：规则守护 (Rule Guard)（Tmux-Orchestrator）
...
```
Architect  → 需要强推理能力 → 用Claude Sonnet/Opus
Executor   → 需要快+便宜     → 用Codex/Claude Haiku
Scout      → 只读探索，轻量即可 → 用轻量模型
Reviewer   → 需要细节注意力 → 用强推理模型
```

Overstory把这个映射做成了可配置的：
